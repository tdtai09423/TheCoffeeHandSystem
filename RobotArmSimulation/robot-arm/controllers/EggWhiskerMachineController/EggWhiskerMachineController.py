from machine.EggWhiskerMachine import EggWhiskerMachine
import pika
import threading
import json
import time
...
def rabbitmq_listener(egg_whisker_machine):
    ...
    def callback(ch, method, properties, body):
        ...
        try:
            ...
            if machine == "egg whisker machine":
                process_command(egg_whisker_machine, activity_id, machine, mode, parameters, sequence, ch)
    ...
    print("[Egg Whisker Machine] Waiting for messages from send_command_exchange...")
...
def main():
    egg_whisker_machine = EggWhiskerMachine()

    threading.Thread(target=rabbitmq_listener, args=(egg_whisker_machine,), daemon=True).start()

    while egg_whisker_machine.robot.step(TIME_STEP) != -1:
        pass
